...
PYBIND11_MODULE(game_interfaces_binding, m)
{
	...
	py::class_<GameAction>(m, "GameAction")
		.def(py::init<CraftsmanID, ActionType, SubActionType>())
	...
	py::class_<MapState>(m, "MapState")
		...
		.def("tileExists", &MapState::tileExists)
		.def("setTile", &MapState::setTile)
		.def("setBit", &MapState::setBit)
		.def("clearBit", &MapState::clearBit)
		.def("isBitToggled", &MapState::isBitToggled)
		.def("isAnyOfMaskToggled", &MapState::isAnyOfMaskToggled)
		.def("clearMapBit", &MapState::clearMapBit)
		.def("getTile", &MapState::getTile)
	...
	py::class_<GameState>(m, "GameState")
		.def(py::init<MapState, std::unordered_map<CraftsmanID, Craftsman>>())
		.def("applyActions", &GameState::applyActions);

	py::class_<Game>(m, "Game")
		.def(py::init<GameOptions, std::vector<std::vector<uint32_t>>, std::vector<Craftsman>>())
		.def("addAction", &Game::addAction)
